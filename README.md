...
│
├── 🧹 輔助腳本
│   ├── clean_all_results.sh            # 清除 artifacts/plots/日誌/彙總
│   ├── post_process_all.sh             # 對現有 artifacts_* 產生報告與圖表
│   └── sync_latest.sh                  # 檢查遠端並自動 git pull --rebase
│
├── 🧪 範例輸出 (examples/)
...
- 每次修改前先拉取最新提交，避免與他人產生衝突：
  ...
  git pull --rebase
  # 或使用腳本自動檢查遠端並同步
  ./sync_latest.sh              # 預設同步 origin/main
  ./sync_latest.sh origin dev   # 自訂遠端/分支
  ```
- 要重跑實驗可先執行 `bash clean_all_results.sh` 清理舊 artifacts/plots/log；訓練完成後可用 `bash post_process_all.sh` 自動產出指標、報告與解釋圖表。
